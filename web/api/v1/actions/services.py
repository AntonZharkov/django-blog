...
class LikeDislikeService:
    ...
    def get_instance(self) -> Article | Comment:
        return apps.get_model(app_label="blog", model_name=self.model_name).objects.get(
            pk=self.object_id
        )
    ...
    def get_sum_likes(self) -> int:
        return LikeDislike.objects.filter(
            content_type=ContentType.objects.get_for_model(self.get_instance()),
            object_id=self.get_instance().id,
