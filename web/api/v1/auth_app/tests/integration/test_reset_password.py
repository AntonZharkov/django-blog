...
def test_reset_password(client, user):
    ...
    link_change_password = re.search(
        r"href=[\"]?([^\">]+)", str(email_message.message())
    )[1]
    assert link_change_password
    print(
        f"THIS IS LINK:{link_change_password}"
    )  # почему & кодируется как &amp; в ссылке http://localhost:8000/password-change/?uid=MQ&amp;token=bqib6g-a771a6fa5166f77ffddef413e89cfdc3
    ...
    reset_password_confirm_response = client.post(
        url_change_password, data=data_confirm, format="json"
    )
    assert reset_password_confirm_response.status_code == 200

    # Test login with new password
    url = reverse("api:v1:auth_app:sign-in")
    data_login = {"email": user.email, "password": data_confirm["password_1"]}
