...
def test_article_create(
    api_client, category, language, text_admin, text_user, image_content_file
):
    ...
    response = api_client.post(
        url, data=data, format="multipart/form-data", headers=headers
    )
    ...
    link_moderate_article = re.search(
        r'<p>.*?<a\s+href="(.*?)">', email_admin_message, re.DOTALL
    )[1]
    assert (
        link_moderate_article
        == f"http://localhost:8000/admin/blog/article/{article.id}/change"
    )

    body_admin_message = re.findall(
        r"<body>(.*?)<\/body>", email_admin_message, re.DOTALL
    )[0]
    ...
    body_user_message = re.findall(
        r"<body>(.*?)<\/body>", email_user_message, re.DOTALL
    )[0]
